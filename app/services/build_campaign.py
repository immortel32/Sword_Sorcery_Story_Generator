...
class Campaign:
    ...
    def _include_quest(self, quest: Quest):
        log.info(
            f"Method = _include_waypoint {quest.title}, contains {len(quest.stories)} stories"
        )
        chapter_index = {}

        # Generating the chapter number for all indexes of the sub-story
        for i in range(0, len(quest.stories)):
            # Pre-Generate the chapter index for each story
            index = self._get_chapter_index()
            log.info(f"   Generating index {i + 1} = {index}")
            chapter_index[f"INDEX_{i + 1}"] = index
        ...
        # Adding chapters to the book of secret
        for story in quest.stories:
            chapter = Chapter()
            chapter.id = chapter_index[f"INDEX_{story.index}"]
            chapter.title = quest.title
            chapter.text = story.text
            while chapter.text.find("<<INDEX") != -1:
                replace_index = chapter.text[
                    (chapter.text.find("<<INDEX") + 2) : chapter.text.find(">>")
        ...
        # Creating instructions
        if quest.instruction_setup:
            instruction = Instruction()
            instruction.source = "event_name"
            instruction.instruction = quest.instruction_setup
            self.book_of_secret.instruction_setup.append(instruction)

    # -----------------------------------------

    def build_campaign(self, waypoint_list: List[Waypoint], quest_list: [Quest]):

        i = 1

        for quest in quest_list:
            self._include_quest(quest)
